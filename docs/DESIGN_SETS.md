...
export default {
  ...
  darkColors: { /* 任意。定義するとダークモード用 @media が自動生成される */ },
...
`darkColors` を定義すると `applyTheme()` が `@media (prefers-color-scheme: dark)` ブロックを自動生成し、
OS のダークモード設定に応じてカラートークンが切り替わる。`darkColors` のキーは `colors` と同じ。
...
実データ由来の確定値。
```
ライト  --c-base:#f9f9f9  --c-surface:#ffffff  --c-primary:#8b0000(sangria)
        --c-accent:#C85A32(terracotta)  --c-support:#556B2F(olive)
        --c-text:#2d2d2d  --c-text-muted:#7a7a7a  --c-border:#ececec
ダーク  --c-base:#1a1a1a  --c-surface:#242424  --c-primary:#e87878
        --c-accent:#e07a52  --c-support:#7aaa4a
        --c-text:#e8e8e8  --c-text-muted:#9a9a9a  --c-border:#333333
...
実データ由来の確定値。
```
ライト  --c-base:#EBF5FA  --c-surface:#ffffff  --c-primary:#003A5C(ocean-deep)
        --c-accent:#E8562A(coral)  --c-support:#00897B(teal)
        --c-text:#1A2E3A  --c-text-muted:#5A7080  --c-border:#e0f2fe(sky-100)
ダーク  --c-base:#0d1b24  --c-surface:#162332  --c-primary:#5ab8e8
        --c-accent:#f08050  --c-support:#3dc4b4
        --c-text:#dceef8  --c-text-muted:#7a9aaa  --c-border:#1e3040
...
起点値（実装時に微調整可）。
```
ライト  --c-base:#f4f4f5  --c-surface:#ffffff  --c-primary:#18181b
        --c-accent:#3f3f46  --c-support:#71717a
        --c-text:#18181b  --c-text-muted:#71717a  --c-border:#e4e4e7
ダーク  --c-base:#0f0f10  --c-surface:#18181b  --c-primary:#f4f4f5
        --c-accent:#d4d4d8  --c-support:#a1a1aa
        --c-text:#fafafa  --c-text-muted:#a1a1aa  --c-border:#27272a
...
起点値（実装時に微調整可）。
```
ライト  --c-base:#f5f1e8  --c-surface:#fffdf8  --c-primary:#5b4636(墨/土)
        --c-accent:#9c6b3f(枯茶)  --c-support:#6a7f4f(苔)
        --c-text:#3a352e  --c-text-muted:#8a8072  --c-border:#e6ddca
ダーク  --c-base:#1e1a16  --c-surface:#2a2420  --c-primary:#d4a878
        --c-accent:#c49060  --c-support:#8fab70
        --c-text:#e8e0d4  --c-text-muted:#a09880  --c-border:#3e3428
...
3. `darkColors` を追加してダークモード配色を定義（任意だが推奨）
4. `googleFonts` に使用書体を列挙（未使用セットのフォントは注入されない）
5. 必要なら `icons` / `badgeTones` を上書き
6. `config.design:"xxx"` で全 examples が破綻せず描画されるか確認
